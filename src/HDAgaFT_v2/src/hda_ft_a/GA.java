...
public class GA extends Thread{
	...
	/** embedded HDA life cycle duration */
	int duration=0;
		...
		if(!reject) {
			for(int l=0;l<cnet.getgraph().length;l++) {
				if(cnet.getgraph()[l]>0) {	
					...
					double tt=(vnhops*minband)-(cnet.getedgew()[l]/1000.0);
					ff+=tt;
					}
				}
			}
				...
				if(cnet.getgraph()[l]>0) {	
					...
					if(mapping[t1[0]]!=mapping[t1[1]]) {
						...
						if(minband<(cnet.getedgew()[l]/1000.0)) {
							...
							ff+=normaliz2*((vnhops*minband)-(cnet.getedgew()[l]/1000.0));
						}
					}
				}
			}
