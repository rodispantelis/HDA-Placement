...
/** Fat-Tree Network class; network nodes are servers; links connect switches and servers;
 *  every Virtual Node (VN) of a HDA-graph is embedded to a single network node */
...
public class FTnetwork {
	...
	ArrayList<HDA> embeddedHDAs=new ArrayList<HDA>();	
	...
	int maxhop=0;		
	/** print on screen embedded HDA statistics */
	boolean printHDAstats=false;	
	...
	/** network orchestrator object; create an orchestrator object and cast it here */
	...
	/** get number of embedded VNs */
	public int getnumofembedded() {
		return embeddedHDAs.size();
	}
	...
	/** check the validity of embedding of input HDA-graph based on generated mapping */
	public boolean checkembed(HDAgraph hdagraph, int[] mapping) {
		...
		for(int n=0; n<hdagraph.getnodes();n++) {
			if((hdagraph.getnodew()[n]+tempload[getserver(mapping[n]).getid()]+
					getserver(mapping[n]).getcpuload()) >(getserver(mapping[n]).getcpu())) {
				invalid=true;
				errmess="CPU constraints. node:"+getserver(mapping[n]).getid();
			}else {
				tempload[getserver(mapping[n]).getid()]+=(hdagraph.getnodew()[n]);
			}
		}
		
		tempload=null;	
		
		if(!invalid && hdagraph.hasspatial()) {
			...
			for(int i=0;i<hdagraph.getsegbands().length;i++) {
				if(hdagraph.getsegflows()[i]>0) {
					...
					for(int tt=0;tt<(t2.length-1);tt++) {
						if(t2[tt]!=t2[tt+1]) {
							...
							int tempve=cod.coder(t2[tt],t2[tt+1]);
							tempbandw[tempve]+=(0.0+(hdagraph.getsegbands()[i]/1000.0));
		...
		if(!invalid && !hdagraph.hasspatial()) {
			for(int l=0;l<hdagraph.getgraph().length;l++) {
				...
				tempbandw1.set(index, (tempbandw1.get(index)+(hdagraph.getedgew()[l]/1000.0)));
	...
	/** embed input HDA-graph based on generated mapping */
	public void embed(HDAgraph hdagraph, int[] mapping) {

		reject=false;
		lastrequestsize=hdagraph.getnodes();

		Double vntr=0.0;	//traffic that the VN adds to the network
		Double inserver=0.0;
		int hops=0;
		...
		if(!reject) {
			...
			ArrayList<Double[]> embeddedcpu=new ArrayList<Double[]>();//cpu demands of embedded VNs
			ArrayList<Double[]> embeddedband=new ArrayList<Double[]>();//bandwidth demands of embedded VNs

			for(int n=0; n<hdagraph.getnodes();n++) {
				getserver(mapping[n]).addcpuload(0.0+hdagraph.getnodew()[n]);
				getserver(mapping[n]).addvnf();
				Double[] temp= {mapping[n]+0.0,hdagraph.getnodew()[n]+0.0};
		...
		//and bandwidth
			for(int l=0;l<hdagraph.getgraph().length;l++) {
				if(hdagraph.getgraph()[l]>0) {
					int vnhops=0;Double tvntr=0.0;
					int[] t1=cod.decoder(l);
					int[] t2=getserverpath(mapping[t1[0]],mapping[t1[1]]);

					for(int tt=0;tt<(t2.length-1);tt++) {
						if(t2[tt]!=t2[tt+1]) {
							links.get(cod.coder(t2[tt],t2[tt+1])).addload(hdagraph.getedgew()[l]/1000.0);
							tvntr+=0.0+(hdagraph.getedgew()[l]/1000.0);//demands of embedded virtual link
							Double[] temp= {cod.coder(t2[tt],t2[tt+1])+0.0,(hdagraph.getedgew()[l]/1000.0)};
							embeddedband.add(temp);
						}else {
							inserver+=(hdagraph.getedgew()[l]/1000.0);
						}
					}
					
					vnhops=(t2.length-2);
					hops+=vnhops;
					vntr+=tvntr*vnhops;	//demands*hop-count
				}
						...
						if(hdagraph.getsegflows()[i]>0) {
							...
							for(int tt=0;tt<(t21.length-1);tt++) {
								if(t21[tt]!=t21[tt+1]) {
									...
									links.get(cod.coder(t21[tt],t21[tt+1])).addload(hdagraph.getsegbands()[i]/1000.0);
									tvntr+=0.0+(hdagraph.getedgew()[l]/1000.0);//demands of embedded virtual link
									
									Double[] temp= {cod.coder(t21[tt],t21[tt+1])+0.0,(hdagraph.getsegbands()[i]/1000.0)};
									embeddedband.add(temp);	
								}
							}
			...
			for(int nr=0;nr<hdagraph.getnodew().length;nr++) {
				noderev+=hdagraph.getnodew()[nr];
			}
		
			//compute virtual link revenue
			for(int nl=0;nl<hdagraph.getedgew().length;nl++) {
				if(hdagraph.getgraph()[nl]==0) {		//if there is no link
					
				}else if(hdagraph.getgraph()[nl]==3){	//if link is bidirectional
					linkrev+=2.0*hdagraph.getedgew()[nl]/1000.0;
				}else {									//if link is single direction
					linkrev+=0.0+(hdagraph.getedgew()[nl]/1000.0);
				}
			}
			
			//revenue of VN, it accumulates all the node and link capacity demands
			revenue=(wn*noderev)+(wl*linkrev);
			embeddedHDAs.add(new HDA(embeddedcpu, embeddedband, duration, hdagraph.getbanddemand(), inserver, iteration));
			reqrevenue=revenue;

			//cost accumulates embedding cost of all nodes and substrate link
			reqcost=(wn*noderev)+(wc*vntr);
		}else{
		...
		if(printHDAstats) {
			vnfstats(revenue, noderev, vntr, hops);
		}
	}
	
	/** get embedded HDAs */
	public ArrayList<HDA> getembeddedHDAs(){
		return embeddedHDAs;
	}
	
	/** get single embedded VN */
	public HDA getvnf(int i) {
		return embeddedHDAs.get(i);
	}
	...
	/** remove embedded HDA for multi-PoP infrastructures */
	public void delembeddedbyid(int hdaid) {
		
		int rvn=-1;
		
		for(int i=0;i<embeddedHDAs.size();i++) {
			if(embeddedHDAs.get(i).getid()==hdaid) {
				rvn=i;

				int t0=embeddedHDAs.get(rvn).getcpu().size();
			
				for(int i1=0;i1<t0;i1++) {
					getserver(embeddedHDAs.get(rvn).getcpu().get(i1)[0].intValue())
						.remcpuload(embeddedHDAs.get(rvn).getcpu().get(i1)[1]);
					getserver(embeddedHDAs.get(rvn).getcpu().get(i1)[0].intValue()).remvnf(1);
				}
		
				int t1=embeddedHDAs.get(rvn).getband().size();
				for(int i2=0;i2<t1;i2++) {
					links.get(embeddedHDAs.get(rvn).getband().get(i2)[0].intValue())
						.remload(embeddedHDAs.get(rvn)
								.getband().get(i2)[1]);
				}
		
				embeddedHDAs.remove(rvn);
			}
		}
	}
	...
	public void vnfstats(Double revenue, Double noderev, Double vntr, int hops) {
		System.out.println("\n");
		System.out.println("VN revenue:\t\t"+revenue);
		
		Double cost=noderev+vntr;
		System.out.println("VN cost:\t\t"+cost);
		System.out.println("VN hops:\t\t"+hops);
	...
	/** get available cpu; considers the servers that can host the minimum demand VN */
	...
	public void netstats() {
		...
		//total number of hosted VNs in all servers
		for(int s1=0;s1<racks.size();s1++) {
			for(int s2=0;s2<servperrack;s2++) {
		...
		for(int e=0;e<embeddedHDAs.size();e++) {
			inserver+=embeddedHDAs.get(e).getinserver();
		}
		...
		results=Integer.toString(requests)+";"+					//1.Request serial number
				Integer.toString(v)+";"+						//2.Hosted VNs
				Integer.toString(getnumofembedded())+";"+		//3.Embedded Hyper-Distributed Applications
				df.format(band)+";"+							//4.Used bandwidth
				df.format(rband)+";"+							//5.Available bandwidth
				df.format(cpu)+";"+								//6.Used cpu
				Integer.toString(a)+";"+						//7.Used servers; servers that host some HDA component
				...
				Integer.toString(lastrequestsize)+";"+			//18.Size of HDA-graph
		...
		if(printnetstats) {
			System.out.println("\nNetwork statistics "+dname+"\n"+""
				+"Hosted VNs:\t\t"+v+"\n"
				+"Embedded HDA-chains:\t"+getnumofembedded()+"\n"
	...
	public void vnfstats() {
		...
		for(int vs=0;vs<embeddedHDAs.size();vs++) {
			a+=embeddedHDAs.get(vs).getcpu().size();
		}

		for(int vs0=0;vs0<embeddedHDAs.size();vs0++) {
			for(int vs1=0;vs1<embeddedHDAs.get(vs0).getcpu().size();vs1++) {
				cpu+=embeddedHDAs.get(vs0).getcpu().get(vs1)[1];
			}
			
			for(int vs2=0;vs2<embeddedHDAs.get(vs0).getband().size();vs2++) {
				band+=embeddedHDAs.get(vs0).getband().get(vs2)[1];;
			}
		}
		
		System.out.println("\nVN statistics\n"+""
				+"Hosted VNs:\t\t"+a+"\n"
				+"Embedded HDA-chains:\t"+embeddedHDAs.size()+"\n"
