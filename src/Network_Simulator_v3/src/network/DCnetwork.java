...
/** Data Center network implements a network of Points of Presence;
 * every node is a Data Center (DC) of three layer Fat-Tree topology;
 * every Virtual Node (VN) of a HDA-graphs is deployed to a single DC;
 * the links of the network interconnect DCs. */
public class DCnetwork {
	...
	/** Virtual Node VN lifecycle duration */
	int duration=0;											
	...
	public Object HPorchestrator;
	/** hop counts */
	int hops=0; int vnhops=0;
	...
	public DCnetwork(String type, FTnetwork[] nets, int[] evi, double interlinkcap, int links) {
		this.type=type;
		this.nets=nets;
	...
	/** get network orchestrator */
	public Object getorchestrator() {
		return HPorchestrator;
	}
	...
	public boolean checkembed(HDAgraph hdagraph, int[] mapping) {
		...
		for(int n=0; n<hdagraph.getnodes();n++) {
			double tvl=hdagraph.getnodew()[n]+tempload[nets[mapping[n]].getid()];
			
			if(tvl >(nets[mapping[n]].getavailablenodecpu()[0])) {
				invalid=true;
				errmess=errmess+("CPU constraints. node:"+nets[mapping[n]].getid());
			}else {
				tempload[nets[mapping[n]].getid()]+=(hdagraph.getnodew()[n]);
			}
		}
	...
	/** remove embedded HDA for multi-domain infrastructures */
	public void delembeddedbyid(int hdaid) {
		
		int rvn=-1;
		
		for(int i0=0;i0<embeddedHDAs.size();i0++) {
			if(embeddedHDAs.get(i0).getid()==hdaid) {
				rvn=hdaid;
				if(rvn>(-1)) {
					for(int n=0;n<nets.length;n++) {
						nets[n].delembeddedbyid(hdaid);
					}
		
					for(int i1=(embeddedHDAs.size()-1);i1>=0;i1--) {
						if(embeddedHDAs.get(i1).getid()==hdaid) {
							Double[] t1=new Double[embeddedHDAs.get(i1).getband().size()];
								for(int i=0;i<t1.length;i++) {
									links.get(embeddedHDAs.get(i1).getband().get(i)[0].intValue())
										.remload(embeddedHDAs.get(i1).getband().get(i)[1]);
								}
								embeddedHDAs.remove(i1);
						}
					}
				}
			}
	...
	public void vnfstats(Double revenue, Double noderev, Double vntr, int hops) {
		System.out.println("\n");
		System.out.println("HDA revenue:\t\t"+revenue);
		
		Double cost=noderev+vntr;
		System.out.println("HDA cost:\t\t"+cost);
		System.out.println("HDA hops:\t\t"+hops);
	...
	public void netstats() {
		...
		results=Integer.toString(requests)+";"+					//1.Request serial number
				...
				Integer.toString(a)+";"+						//6.Used servers; servers that host some VN
				...
				(vnhops+hops)+";"+								//17.Request hop count
				(iteration+1);									//18.Request ID
